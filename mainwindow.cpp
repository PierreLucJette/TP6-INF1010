...
void MainWindow::setConnection() {
    ...
    QObject::connect(exit_action_, SIGNAL(triggered()), this, SLOT(close()));
	...
	//    cf. la class QAction
    QObject::connect(open_action_, SIGNAL(triggered()), this, SLOT(load()));
    ...
    QObject::connect(process_combobox_, SIGNAL(currentIndexChanged(int)), this, SLOT(processImage()));
    ...
    QObject::connect(slider_, SIGNAL(valueChanged(int)), this, SLOT(processImage()));
    ...
    QObject::connect(empty_button_, SIGNAL(clicked()), this, SLOT(empty()));
    ...
    QObject::connect(list_paths_, SIGNAL(itemClicked(QListWidgetItem*)), this, SLOT(setImage(QListWidgetItem*)));
}
...
void MainWindow::checkImageValid(const QImage &image) const {
    ...
    try{
        if(image.isNull())
            throw ImageLoadingException("Error: Image Not Valid \"ImageLoadingException\"");
...
void MainWindow::checkImageAlreadyLoaded(const QImage &image) const {
    ...
    try{
        if(images_list_.contains(image))
            throw ImageLoadingException("Error: Image Already Present in List \"ImageLoadingException\"");
    } catch(ImageLoadingException){
        throw;
    }
}
...
void MainWindow::processImageIntensity() {
    ...
    try{
        if(process_.intensity(slider_->value(), images_list_[index_], image_process_) > 0)
            throw ImageProcessingException("Error: Image's Mean Intensity Too High \"ImageProcessingException\"");
    } catch(ImageProcessingException){
        throw;
    }
    try{
        if(process_.intensity(slider_->value(), images_list_[index_], image_process_) < 0)
            throw ImageProcessingException("Error: Image's Mean Intensity Too Low \"ImageProcessingException\"");
    } catch(ImageProcessingException){
        throw;
    }
}
...
void MainWindow::processImageContrast() {
    ...
    try{
        if(process_.contrast(slider_->value(), images_list_[index_], image_process_) > 0)
            throw ImageProcessingException("Error: Image's Mean Contrast Too High \"ImageProcessingException\"");
    } catch(ImageProcessingException){
        throw;
    }
    try{
        if(process_.contrast(slider_->value(), images_list_[index_], image_process_) < 0)
            throw ImageProcessingException("Error: Image's Mean Constrast Too Low \"ImageProcessingException\"");
    } catch(ImageProcessingException){
        throw;
    }
}
